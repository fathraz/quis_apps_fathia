...
export class Tab3Page {
  ...
  async getData()
  {
    ...
    try
    {
      const res = await axios.post('http://localhost/api_tugas/get_data_mahasiswaId.php', fd);
  ...
  async updateData()
  {
    ...
    try 
    {
      const res = await axios.post('http://localhost/api_tugas/update_data_mahasiswa.php', fd);
      if(res.data.error == false)
      {
        alert('Berhasil menupdate data');
        this.dismiss();
